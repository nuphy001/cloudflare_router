...
const HEADLESS_ROUTES = {
  ...
  prefixes: [], // /account 默认走无头商店
  patterns: [],
};

// 最简单路由匹配：仅判断是否在 exact 集合中
function isHeadlessRoute(path) {
  // /account/login/multipass/ 走在线商店（Multipass 必须）
  if (path.startsWith("/account/login/multipass/")) {
    return false;
  }
...
function shouldBypass(url) {
  ...
  // 针对 .myshopify.com/checkouts/ 的显式兜底
...
async function handleRequest(request) {
  ...
  try {
    ...
    if (path.startsWith("/account/login/multipass/")) {
      ...
      // 如果 Shopify 返回重定向（302/307）
      if (response.status >= 300 && response.status < 400) {
        const location = response.headers.get("location");
        if (location) {
          // 重写重定向地址，将 myshopify.com 替换为 nuphy.ai
          let newLocation = location
            .replace(
              new RegExp(`https://${SHOPIFY_ORIGIN_DOMAIN}`, "gi"),
              `https://${url.hostname}`
            )
            .replace(
              new RegExp(`http://${SHOPIFY_ORIGIN_DOMAIN}`, "gi"),
              `https://${url.hostname}`
            );

          console.log(`🔀 Multipass 重定向: ${location} -> ${newLocation}`);

          // 重写 Cookie domain
          const newHeaders = new Headers(response.headers);
          newHeaders.set("location", newLocation);

          // 处理 Set-Cookie headers
          ...
          if (setCookieHeaders && setCookieHeaders.length > 0) {
            ...
            setCookieHeaders.forEach((cookie) => {
              ...
              if (rewrittenCookie && !rewrittenCookie.includes("domain=")) {
                rewrittenCookie += `; domain=.${url.hostname}`; // 设置为 .nuphy.ai
              }
...
// 通用响应重写函数
async function rewriteResponse(response, originalUrl) {
  try {
    const newHeaders = new Headers(response.headers);
    const escapedDomain = SHOPIFY_ORIGIN_DOMAIN.replace(/\./g, "\\.");
...
async function rewritePasswordPageResponse(response, originalUrl) {
  return rewriteResponse(response, originalUrl);
}
